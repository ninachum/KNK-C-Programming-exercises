...
void print(void)
{
	...
	qsort(inventory, num_parts, sizeof(struct part), compare_part_number);
...
void dump(void)
{
	...
	qsort(inventory, num_parts, sizeof(struct part), compare_part_number);
	if (fwrite(inventory, sizeof(struct part), num_parts, dump_file) != (unsigned int)num_parts) {
		printf("error: partly written, due to write error \n");
		exit(EXIT_FAILURE);
	}
...
void restore(void)
{
	...
	num_parts = fread(inventory, sizeof(struct part), num_parts, dump_file);

	if (num_parts > MAX_PARTS) {
		printf("There are over MAX_PARTS in that file. can't open.\n");
		exit(EXIT_FAILURE);
	}
